...
use app\common\pluings\sms\Driver;
...
class Tencent implements SmsInterface
{
     ...
     * CAM密匙查询: https://console.cloud.tencent.com/cam/capi*/
        ...
        $cred = new Credential($this->options['secretId'], $this->options['secretKey']);
        ...
        $req->SmsSdkAppId = $this->options['smsSdkAppId'];
        /* 短信签名内容: 使用 UTF-8 编码，必须填写已审核通过的签名，签名信息可登录 [短信控制台] 查看 */
        $req->SignName = $this->options['signName'];
        ...
        try{
            if($resp->SendStatusSet[0]->Code=='Ok'){
                return (new SendResult())->status(true)->originalContent($resp);
            }else{
                UtilsLog::record($resp,"Error-Tencent-Sms");
                exception("短信发送失败");
            }
        }catch (\Exception $e){
            UtilsLog::record($resp,"Error-Tencent-Sms");
